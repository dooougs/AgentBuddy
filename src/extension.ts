...
// All the agent stages we can detect or infer
...
export function activate(context: vscode.ExtensionContext) {
  const panel = new BuddyPanel(context);

  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('agentBuddy.panel', panel, {
  ...
  let editBurstTimer: NodeJS.Timeout;

  const setStage = (stage: AgentStage, meta?: Record<string, unknown>) => {
    lastStage = stage;
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(() => {
      ...
      if (lastStage === 'editing') {
        ...
        setTimeout(() => {
          if (lastStage === 'thinking') setStage('idle');
        }, 2000);
      }
    })
  );
  ...
  context.subscriptions.push(
    vscode.window.onDidChangeActiveTextEditor((editor) => {
      ...
      if (lastStage !== 'editing' && lastStage !== 'terminal') {
        setStage('searching');
        setTimeout(() => {
          if (lastStage === 'searching') setStage('thinking');
        }, 1500);
      }
    })
  );
  ...
  const stages: AgentStage[] = ['thinking', 'planning', 'searching', 'editing', 'terminal', 'success', 'idle', 'sleeping'];
  let simIdx = 0;
  context.subscriptions.push(
    vscode.commands.registerCommand('agentBuddy.simulate', () => {
